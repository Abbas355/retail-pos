...
19. voice_sale (use intent "create_sale") – Your responsibility: understand the context, listen to the WHOLE message, and extract EACH product mentioned. Do not take one or two products – take EVERY product from the message. A single message may contain MULTIPLE products; all belong to the SAME sale. Never return only one or two items when the user said three or more (e.g. "3 breads 2 eggs 1 Coca-Cola" must yield three items).
...
- Extract intent from natural language. Examples:
  ...
  - "Talha ka khata bata do kitna rehta hai" / "mujhe Talha ka khata bata do" / "Ali ka balance" (with name) → {"action":"khata_customer","customerName":"Talha"} or customerName:"Ali"
...
function parseIntentResponse(text) {
  ...
  try {
    ...
    if (action === "khata_customer") {
      const customerName = obj.customerName != null ? String(obj.customerName).trim() : (obj.customer_name != null ? String(obj.customer_name).trim() : "");
      if (!customerName) return { action: "unknown" };
      return { action: "khata_customer", customerName: sanitizeCustomerName(customerName) || customerName };
    }
    return null;
  } catch (_) {
    return null;
