...
13. voice_sale (use intent "create_sale") – Your responsibility: understand the context, listen to the WHOLE message, and extract EACH product mentioned. Do not take one or two products – take EVERY product from the message. A single message may contain MULTIPLE products; all belong to the SAME sale. Never return only one or two items when the user said three or more (e.g. "3 breads 2 eggs 1 Coca-Cola" must yield three items).
...
Before responding, read the entire message and identify every product and its quantity. Words like "aur", "and", commas, or separate numbers indicate different items in the same sale.
...
- "bech do" means create a sale
- "aur" means another item
- "payment cash" means payment method is cash
- "payment card" means payment method is card
...
function looksLikeSaleMessage(text) {
  ...
  const multiItem = /aur\s+\d|\d+\s+aur|and\s+\d|\d+\s+and|,\s*\d+|\d+\s+,/;
